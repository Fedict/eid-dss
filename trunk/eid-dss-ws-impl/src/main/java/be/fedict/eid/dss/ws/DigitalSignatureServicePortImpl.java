...
public class DigitalSignatureServicePortImpl implements
    ...
    public ResponseBaseType verify(VerifyRequest verifyRequest) {
        ...
        List<DocumentDO> documents =
                getDocuments(verifyRequest.getInputDocuments());
    ...
    public SignResponse sign(SignRequest signRequest) {
        ...
        for (Object optionalInputObject : optionalInputs.getAny()) {
            ...
            if (optionalInputObject instanceof Element) {
                ...
                if (DSSConstants.ARTIFACT_NAMESPACE
                        .equals(element.getNamespaceURI()) &&
                        DSSConstants.RETURN_STORAGE_INFO
                                .equals(element.getLocalName())) {

                    returnStorageInfo = true;

                } else if (DSSConstants.ARTIFACT_NAMESPACE
                        .equals(element.getNamespaceURI()) &&
                        DSSConstants.RETURN_STORED_DOCUMENT
                                .equals(element.getLocalName())) {

                    try {
                        ReturnStoredDocument returnStoredDocument =
                                DSSUtil.getReturnStoredDocument(element);
        ...
        if (!returnStorageInfo && null == documentId ||
                returnStorageInfo && null != documentId) {
    ...
    private SignResponse storedDocumentResponse(SignRequest signRequest,
        ...
        DocumentWithSignature documentWithSignature =
                dssObjectFactory.createDocumentWithSignature();
    ...
    private SignResponse storageInfoResponse(SignRequest signRequest) {
        ...
        if (documents.isEmpty()) {
            return DSSUtil.createRequestorSignErrorResponse(signRequest.getRequestID(),
                    null, "No valid document found to validate.");
        }
        if (documents.size() != 1) {
            return DSSUtil.createRequestorSignErrorResponse(signRequest.getRequestID(),
                    null, "Can validate only one document.");
        }

        // store artifact
        String documentId = UUID.randomUUID().toString();

        DateTime expiration = this.documentService.store(
                documentId,
                documents.get(0).getDocumentData(),
                documents.get(0).getContentType());


        // construct response
        ObjectFactory dssObjectFactory = new ObjectFactory();
        be.fedict.eid.dss.ws.profile.artifact.jaxb.ObjectFactory artifactObjectFactory
